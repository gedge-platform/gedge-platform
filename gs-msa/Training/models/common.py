...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        #print("CSP : ", self.cv4(self.act(self.bn(torch.cat((y1, y2), dim=1)))).shape)
...
class BottleneckCSP2(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x1)
        #print(y2)
        #print("CSP2 : ", self.cv3(self.act(self.bn(torch.cat((y1, y2), dim=1)))).shape)
...
class SPP(nn.Module):
    ...
    def forward(self, x):
        x = self.cv1(x)
        #print("SPP : ", self.cv2(torch.cat([x] + [m(x) for m in self.m], 1)).shape)
...
class SPPCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        #print("SPPCSP : ", self.cv7(self.act(self.bn(torch.cat((y1, y2), dim=1)))).shape)

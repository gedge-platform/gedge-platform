...
def test(data,
    ...
    if training:  # called by train.py
        ...
        #gs = int(max(model.imgstride)) #test large&medium
        gs = int(max(model.stride)) #test original
        imgsz2 = check_img_size(imgsz, gs)  # check img_size
        #print("test.py test imgsz: ", imgsz)
    ...
    # Dataloader
    if not training:
        imgsz2 = check_img_size(imgsz, gs)
        img = torch.zeros((1, 3, imgsz, imgsz2), device=device)  # init img
        _ = model(img.half() if half else img) if device.type != 'cpu' else None  # run once
        path = data['test'] if opt.task == 'test' else data['val']  # path to val/test images
        stride = 32 if 32 > model.stride.max() else model.stride.max() #test original
        #stride = int(max(model.imgstride)) #test large&medium
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        #height = 384  # !!!!!!!!!!!!!!!!!
        #print("test.py - test : ", height, width)
        ...
        for si, pred in enumerate(output):
            ...
            seen += 1
            #save_path = str(Path(out) / Path(paths[si]).name)
            ...
            if save_txt:
                ...
                #print(Path(paths[si]))
                im0s = cv2.imread(str(Path(paths[si])))
                ...
                for *xyxy, conf, cls in pred:
                    ...
                    with open(txt_path + '.txt', 'a') as f:
                        f.write(('%g, ' * 6 + '-1, ' * 3 + '-1' + '\n') % (frame_num, detection_count, *xyxy))  # label format
                    detection_count+=1

...
def train(hyp, opt, device, tb_writer=None):
    ...
    nc, names = (1, ['person']) if opt.single_cls else (int(data_dict['nc']), data_dict['names'])  # number classes, names
    ...
    # Image sizes
    gs = int(max(model.imgstride))  # grid size (max stride)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if dataset.image_weights:
            ...
            if rank != -1:
                ...
                print("train.py epoch indices : ", indices)
                if rank == 0:

...
const ProjectPhysical = observer((props) => {
    ...
    data.map((item, index) => {
        if (pod_cpu === item.metric.pod) {
            test = item.values
        }
    })
    data2.map((item) => {
        if (pod_memory === item.metric.pod) {
            console.log(item, "발견")
            test2 = item.values
        }
    })
    ...
    data3.map((item) => {
        if (pod_traf_in === item.metric.pod) {
            test3 = item.values
        }
    })
    data4.map((item) => {
        if (pod_traf_out === item.metric.pod) {
            test4 = item.values
        }
    })

    console.log("deployment monitoring kwoo2")
...
export default ProjectPhysical
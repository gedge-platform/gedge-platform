...
function azureStateIconFormatter(cellval, options, row) {

	var statue = "";
	if(row.powerState != null && row.provisioningState != null) {
		statue = row.powerState + "(" + row.provisioningState + ")";
	} else if (row.powerState != null && row.provisioningState == null) {
		statue = row.powerState;
	} else if (row.powerState == null && row.provisioningState != null) {
		statue = row.provisioningState;
	} else {
		"-";
	}

	if(row.powerState == 'running' && row.provisioningState == 'Succeeded') {
		return "<span class='ico_status_running'>" + statue + "</span>";
	} else if(row.provisioningState == 'Failed') {
		return "<span class='ico_status_trouble'>" + statue + "</span>";
	} else if(row.powerState == 'deallocated') {
		return "<span class='ico_status_disabled'>" + statue + "</span>";
	} else {
...
function stateIconFormatter(cellval, options, row) {
	...
	if(typeof row == 'object') {
		if(_.has(row, 'state') || _.has(row, 'powerState') || _.has(row, 'status') || _.has(row, 'status.nodeInfo.status') || _.has(row, 'status.phase')) {
			if(options === 'state' || options.colModel.name === 'state' || options === 'powerState' || options.colModel.name === 'powerState' || options === 'status' || options.colModel.name === 'status' || options.colModel.name === 'status.nodeInfo.state' || options.colModel.name === 'status.phase') {
				if(!cellval)
					return "";
				if(cellval === 'active' || cellval === 'running' || cellval === 'poweredOn' || cellval === 'RUNNING' || cellval === 'Running') {
...
var getSizeToGBFixedTwo = function (cellval, options, row){

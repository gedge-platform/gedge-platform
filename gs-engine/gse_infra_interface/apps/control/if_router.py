...
from apps.control.kubernetes_controller import if_node, if_pod, if_service
...
class InterfaceRouter:
    ...
    def create(self, **data):
        ...
        result = getattr(self._target, f'{interface_name}_create'.lower())(link=copy.copy(self._link), **arguments)
        if not result or not result.get('result'):
            raise result.get('exc')
        result_data = result.get('response')
        # new_obj = self._obj_class(self, **result.get('response'))
